...
namespace ParkMaster3000
{
    public class Parkhaus
    {
        ...
        void ParkstellenGenerator(int anzahlParkDecks, List<int[]> autoMotorradStellen)
        {
            for (var deck = 0; deck > anzahlParkDecks; deck++)
            {
                //Parkdeck Namen beginnend mit A aufsteigend
                char deckName = (char)(deck + 65);
                for (int auto = 0; auto < autoMotorradStellen[deck][0]; auto++)
                {
                ...
                for (int motorrad = autoMotorradStellen[deck][0]; motorrad < autoMotorradStellen[deck][1] + autoMotorradStellen[deck][1]; motorrad++)
                {
